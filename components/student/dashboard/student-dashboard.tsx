...
// Define the shape of the dashboard data (API response)
interface DashboardData {
  user: {
    id: string;
    name: string; // Use name instead of username for display
...
// Define the shape of the transformed dashboard data (client state)
interface TransformedDashboardData {
  ...
  projectSummary: ProjectSummary;
  taskSummary: TaskSummary & {
    upcomingDeadlines: Array<TaskDeadlineData & { deadline: Date }>;
  };
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        if (!data?.user || !data.projectSummary || !data.taskSummary) {
          throw new Error("Invalid data format received from API");
        }

        // Transform API data
        const transformedData: TransformedDashboardData = {
          user: {
            id: data.user.id,
            name: data.user.name || "Student", // Use name from API
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      <Navbar
        ...
        userName={dashboardData?.user?.name || "Student"} // Use name for display
      />
