...
export async function GET() {
  try {
    ...
    const projectSummary = {
      ...
      completedProjects: projects.filter((p) => p.status === "Completed" || p.archived).length,
    };
    ...
    const upcomingDeadlines = tasks
      .filter((task) => task.deadline && task.status !== TaskStatus.DONE && new Date(task.deadline) > now)
      .map((task) => {
    ...
    try {
      ...
      const recentActivities = [
        ...
        ...recentFeedback.map((fb: FeedbackData) => ({
          ...
          title: fb.author.id === userId
            ...
            : `Feedback received from ${fb.author.firstName || 'Anonymous'}`,
      ...
      // Construct user data with name from Prisma or session
      const userData = {
        id: user.id,
        name: user.username || session.user.username || user.username || "Student",
